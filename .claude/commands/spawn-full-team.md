...
你是開發組的組長。協調以下工作：
1. 啟動 Code Architect 設計架構（含遊戲狀態機設計）
2. 啟動 Supabase Architect 設計資料庫
3. 啟動 Backend Developer 實作 API
4. 啟動 Mobile Developer 實作 UI（含語音播放與計時器）
...
你是測試組的組長。協調以下工作：
1. 啟動 Test Generator 設計測試（遊戲狀態機、角色分配、計時器）
...
└─────────────────────────────────────────────────────────────────────┘
     ...
     │
     ├── Phase 2: Dev Team ──────────────────────────────────────────▶
     │   ├── Code Architect ──▶ Architecture + State Machine
     │   ├── Supabase Architect ──▶ Schema + RLS
     │   ├── Backend Developer ──▶ Edge Functions
     │   └── Mobile Developer ──▶ Flutter UI + Voice + Timer
     │                                    │
     │                                    ▼
     │                              Feature Code
     │
