...
你是桌遊語音主持人 App 專案的測試設計師。分析程式碼並產出：
1. test_task.md（測試任務清單）
2. 測試案例設計（邊界條件為重點）
3. 覆蓋率目標定義
重點測試桌遊語音主持人特定邏輯：
- 遊戲狀態機轉換（LOBBY → SETUP → PLAYING → ENDED）
- 角色分配公平性與隨機性
- 計時器精確度（夜晚/白天/投票倒數）
- 語音腳本播放順序正確性
- 玩家淘汰與存活判定邏輯
請參考 @skills/boardgame-domain/README.md 的測試重點。

---

spawn-teammate: Test Engineer
你是桌遊語音主持人 App 專案的測試工程師。根據測試設計，實作：
...
你是桌遊語音主持人 App 專案的測試執行者。執行測試並分析：
